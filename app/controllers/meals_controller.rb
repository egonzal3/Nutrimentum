class MealsController < ApplicationController
  before_action :current_users_information_must_be_meal_user, :only => [:show, :edit_form, :update_row, :destroy_row]

  def current_users_information_must_be_meal_user
    meal = Meal.find(params["id_to_display"] || params["prefill_with_id"] || params["id_to_modify"] || params["id_to_remove"])

    unless current_users_information == meal.user
      redirect_to :back, :alert => "You are not authorized for that."
    end
  end

  def index
    @q = current_users_information.meals.ransack(params[:q])
    @meals = @q.result(:distinct => true).includes(:user, :plate).page(params[:page]).per(10)
  ...
  def destroy_row_from_user
    ...
    redirect_to("/users_informations/#{@meal.eater}", notice: "Meal deleted successfully.")
  end
